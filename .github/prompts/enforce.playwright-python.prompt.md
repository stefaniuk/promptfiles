---
agent: agent
description: Enforce repository-wide compliance with playwright-python.instructions.md
---
...
Enumerate every Playwright Python test artefact in the repository, detect any discrepancies against `playwright-python.instructions.md`, plan the refactor/rework workstream, implement the required changes, and confirm compliance.
...
1. Re-read the relevant sections of `playwright-python.instructions.md` for the test patterns present (locators, assertions, structure, execution, Page Object Model, etc.).
...
> **Note:** On subsequent runs, check whether artefacts from earlier executions (for example `docs/prompt-reports/playwright-python-inventory.md`, `docs/prompt-reports/playwright-python-instructions-alignment-plan.md`) already exist and parse them so progress is cumulative rather than duplicated.

### 1) Build the Playwright Python artefact matrix

1. Produce a table (for example in `docs/prompt-reports/playwright-python-inventory.md`) listing each test file or folder, its role, and the key instruction tags that apply.
2. Highlight high-risk areas (locator patterns, assertion usage, fixture design, flaky test patterns) where divergence is most likely.
...
1. Group findings into actionable workstreams (for example "Locator modernisation", "Assertion alignment", "Page Object extraction", "Flakiness mitigation").
...
3. Store the plan in `docs/prompt-reports/playwright-python-instructions-alignment-plan.md` for traceability.
...
2. Prefer refactors that replace CSS/XPath with role-based locators, add missing assertions, extract Page Objects, or remove anti-patterns.
...
1. Produce a final enforcement report (append to `docs/prompt-reports/playwright-python-instructions-alignment-plan.md`) covering:
...
- Reference instruction identifiers (for example `[PW-PY-LOC-001]`) when explaining discrepancies or fixes.
- Keep activities broken into the steps above; do not skip steps even if the tests appear compliant.
- Prefer automation (linters, Playwright trace analysis) over manual spot checks where feasible.
